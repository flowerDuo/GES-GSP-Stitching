...
{
	string path = R"(F:\Projects\C++\NISwGSP_Stitching\input-42-data\CAVE-eitan_office\e-05.jpg)";
	Mat imgRes = imread(path, 0), gray;
	cvtColor(imgRes, gray, COLOR_BGR2GRAY);
	vector<vector<Point>> contours;
	vector<Vec4i> hierarchy;

	findContours(imgRes, contours, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_NONE, Point());
