...
void TestSP::testLineProcess() {
	...
	for (int i = 0; i < files.size(); i++) {
		...
		Mat imagers = imread(path);
		cvtColor(imagers, imagers, COLOR_BGR2GRAY);
		Mat image;
		//1.调整到HED规定图片大小.
		resize(imagers, image, cv::Size(500, 500 * (double)imagers.rows / imagers.cols));

		//2.HED图片边缘检测
		edgeDetection(image, image, 0.5);
		//3.回归原本大小
		resize(image, image, Size(imagers.cols, imagers.rows));
		//4.细化边缘图像
		thin(image, image, (double)imagers.cols / 500);
		...
		Mat imageSamples = Mat::zeros(image.size(), CV_8UC3);//输出图
		Mat imageDraw = imread(path);
		resize(imageDraw, imageSamples, image.size());
		int i_length = 0;
		...
		imwrite(path_dir + "\\out\\" + "out-" + files[i], imageSamples);
