...
void connectSmallLine(vector<vector<Point>> contours, vector<Vec4i> hierarchy, vector<vector<Point>>& contoursConnected)
{
	...
	for (int j = connectIds.size() - 1; j >= 0; j--) {
		...
		pair<Point, Point> SEPoint, SEPoint2;
		for (int k = 0; k < connectIds[j].size(); k++) {
			SEPoint = findLineMinAndMax(contours[j]);
			SEPoint2 = findLineMinAndMax(contours[connectIds[j][k]]);
			if (isExtend(SEPoint, SEPoint2)) {
...
bool isExtend(pair<Point, Point> pair1, pair<Point, Point> pair2) {
	//距离阈值
	double distThrhold = 20;
	double distUThrehold = 4;
	float minX1 = pair1.first.x, maxX1 = pair1.first.y, minY1 = pair1.second.x, maxY1 = pair1.second.y;
	float minX2 = pair2.first.x, maxX2 = pair2.first.y, minY2 = pair2.second.x, maxY2 = pair2.second.y;

	float distx = min(maxX1, maxX2) - max(minX1, minX2);
	if (max(minX1, minX2) < min(maxX1, maxX2)) {
		//有交集
		if (abs(distx) >= distUThrehold) {
			//相交太多,判断为不是延伸关系
			return false;
		}
	}
		...
		//无交集
		if (abs(distx) >= distThrhold) {
			//相差太远
			return false;
		}
	}
	float disty = min(maxY1, maxY2) - max(minY1, minY2);
	if (min(maxY1, maxY2) - max(minY1, minY2)) {
		//有交集
		if (abs(disty) >= distUThrehold) {
			//相交太多,判断为不是延伸关系
			return false;
		}
	}
