...
const vector<vector<Point>> ImageData::getContentSamplesPoint(vector<double>& weights) const
{
	...
	map<int, vector<Point>> static_sample; //每个线段一定要加的样本点位置
...
/// <returns></returns>
vector<vector<Point>> connectCollineationLine(vector<vector<Point>>& input, vector <double >& lengths_out, map<int, vector<Point>>& static_sample, int image_width, int image_height) {
