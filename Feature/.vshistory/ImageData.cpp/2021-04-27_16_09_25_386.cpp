...
const vector<vector<Point>> ImageData::getContentSamplesPoint(vector<double>& weights) const
{
	...
	connectCollineationLine(res, res, lineslength);
	...
	*/
	...
	linesPolarData.reserve(res.size());
...
vector<vector<Point>> connectCollineationLine(vector<vector<Point>> input, vector<double>& lengths) {
	for (vector<vector<Point>>::iterator iterator = input.begin(); iterator != input.end(); ++iterator) {

	}
	//拟合线段,转成极坐标,加入vector TODO
	pair<double, double> polarData = transRectangular2Polar(line_para);
	linesPolarData.emplace_back(index, polarData);
}
