...
const vector<vector<Point>> ImageData::getContentSamplesPoint(vector<double>& weights) const
{
	...
	res = connectCollineationLine(res, lineslength);

	//7.取采样点
	Mat imageSamples = Mat::zeros(image.size(), CV_8UC3);//输出图
	//存放所有曲线的起终点,采样点数据.0:起点,1:终点,之后是采样点.
	vector<vector<Point>> samplesData;
	samplesData.reserve(contoursLineConnected.size());
	weights.reserve(contoursLineConnected.size());
	...
	*/
	...
	vector<double>::iterator length_iterator = lineslength.begin();
	for (vector<vector<Point>>::iterator iterator = res.begin(); iterator != res.end() && length_iterator != lineslength.end(); ++iterator, ++length_iterator) {
		//1.计算曲线长度
...
/// <summary>
/// 极坐标数据排序.优先角度排序,后距离排序
/// </summary>
/// <param name="a"></param>
/// <param name="b"></param>
/// <returns></returns>
bool sortForPolarData(pair<int, pair<double, double>> a, pair<int, pair<double, double>> b) {
	pair<double, double> a_Polar = a.second;
	pair<double, double> b_Polar = b.second;

	return (a_Polar.second < b_Polar.second || (a_Polar.second == b_Polar.second && a_Polar.first < b_Polar.first));
}
...
vector<vector<Point>> connectCollineationLine(vector<vector<Point>> input, map<int, double>& lengths) {

	double threshold_r = 10;
	double threshold_theta = 5;

	//存放每一条线的极坐标数据,已经对应samplesData的index
	vector<pair<int, pair<double, double>>> linesPolarData;
	linesPolarData.reserve(input.size());
	for (int i = 0; i < input.size(); i++) {
		//拟合线段,转成极坐标,加入vector
		Vec4f line_para;
		fitLine(input[i], line_para, DIST_L2, 0, 1e-2, 1e-2);
		pair<double, double> polarData = transRectangular2Polar(line_para);
		linesPolarData.emplace_back(i, polarData);
	}
	sort(linesPolarData.begin(), linesPolarData.end(), sortForPolarData);

	//将共线的线 index 整合到二维表中.
	vector<vector<int>> connectIndexs;
	connectIndexs.resize(linesPolarData.size());
	for (int i = 0; i < linesPolarData.size(); i++) {
		pair<int, pair<double, double>> itemPolarDataFirst = linesPolarData[i];
		for (int j = i + 1; j < linesPolarData.size(); j++) {
			pair<int, pair<double, double>> itemPolarDataSecond = linesPolarData[j];
			if (abs(itemPolarDataFirst.second.first - itemPolarDataSecond.second.first) < threshold_r
				&& abs(itemPolarDataFirst.second.second - itemPolarDataSecond.second.second) < threshold_theta) {
				//属于共线;
				connectIndexs[itemPolarDataFirst.first].emplace_back(itemPolarDataSecond.first);
			}
		}
	}
	...
	for (int j = connectIndexs.size() - 1; j >= 0; j--) {
		...
		for (int k = 0; k < connectIndexs[j].size(); k++) {
			...
			if (!isParallax(mmPoint, mmPoint2)) {
				...
				map<int, double>::iterator length_k = lengths.find(k);
				if (length_k == lengths.end() || length_k->second == 0) {
					Size2f size = minAreaRect(input[k]).size;
					length += sqrt(size.width * size.width + size.height * size.height);
				}
				else {
...
bool isExtend(pair<Point, Point> mmpair1, pair<Point, Point> mmpair2, pair<Point, Point> sepair1, pair<Point, Point> sepair2) {
	...
	bool isCloseX = false, isCloseY = false;
...
pair<double, double> transRectangular2Polar(Vec4f line) {
