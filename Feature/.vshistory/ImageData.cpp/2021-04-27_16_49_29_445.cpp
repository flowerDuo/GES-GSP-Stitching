...
vector<vector<Point>> connectCollineationLine(vector<vector<Point>> input, vector<double>& lengths) {
	...
	for (int i = 0; i < linesPolarData.size(); i++) {
		for (int j = i; j < linesPolarData.size(); j++) {
		...
		if (i == 0) {
			connectIndexs[j].emplace_back(linesPolarData[i].first);
			continue;
		}

		pair<int, pair<double, double>> pre_itemPolarData = linesPolarData[i - 1];
		if ((pre_itemPolarData.second.first - itemPolarData.second.first) < )
	}
}
