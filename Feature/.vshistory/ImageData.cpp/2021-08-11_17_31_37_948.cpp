...
const vector<vector<Point>> ImageData::getContentSamplesPoint(vector<double>& weights) const
{
	...
	drawContours(imageContours, contours, -1, Scalar(255, 255, 255), 1, 8, hierarchy);
	//将检测到的角点绘制到原图上
	for (int i = 0; i < corners.size(); i++)
	{
	...
	Mat imageTest = Mat::zeros(image.size(), CV_8UC3);//输出图
	...
	*/
	...
	for (vector<vector<Point>>::iterator iterator = res.begin(); iterator != res.end() && i < lineslength.size(); ++iterator, ++i) {
		...
		for (int j = 0; j < (*iterator).size(); j++) {
			circle(imageTest, (*iterator)[j], 1, s, FILLED);
		}
		index++;
	}

	imwrite(*debug_dir + "Samples" + file_name + file_extension, imageTest);
