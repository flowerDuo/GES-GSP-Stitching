...
bool isExtend(pair<Point, Point> mmpair1, pair<Point, Point> mmpair2, pair<Point, Point> sepair1, pair<Point, Point> sepair2) {
	...
	double distThrhold = 0.5;
	//double distUThrehold = 4;
	double distUThrehold = 0.12;
	...
	if (max(minX1, minX2) < min(maxX1, maxX2)) {
		//有交集
		if (abs(distx) >= distUThrehold * minX) {
			//相交太多,判断为不是延伸关系
			return false;
		}
	}
	...
	if (max(minY1, minY2) < min(maxY1, maxY2)) {
		//有交集
		if (abs(disty) >= distUThrehold * minY) {
			//相交太多,判断为不是延伸关系
			return false;
		}
	}
