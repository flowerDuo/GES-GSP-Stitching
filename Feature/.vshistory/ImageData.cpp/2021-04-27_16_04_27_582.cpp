...
const vector<vector<Point>> ImageData::getContentSamplesPoint(vector<double>& weights) const
{
	...
	*/
	...
	vector<double>::iterator iteratorLength = lineslength.begin();
	for (vector<vector<Point>>::iterator iterator = res.begin(); iterator != res.end() && iteratorLength != lineslength.end(); ++iterator, ++iteratorLength) {
		//1.计算曲线长度
