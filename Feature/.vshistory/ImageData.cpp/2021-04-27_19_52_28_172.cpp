...
const vector<vector<Point>> ImageData::getContentSamplesPoint(vector<double>& weights) const
{
	...
	*/
	...
	for (vector<vector<Point>>::iterator iterator = res.begin(), int i = 0; iterator != res.end() && i < res.size(); ++iterator, ++i) {
		...
		map<int, double>::iterator length_iterator = lineslength.find(i);
		if (length_iterator == lineslength.end() || length_iterator->second == 0) {
			Size2f size = minAreaRect(*iterator).size;
			contourLength = sqrt(size.width * size.width + size.height * size.height);
		}
		else {
			contourLength = length_iterator->second;
		}
