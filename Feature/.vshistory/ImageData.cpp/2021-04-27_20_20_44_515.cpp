...
const vector<vector<Point>> ImageData::getContentSamplesPoint(vector<double>& weights) const
{
	...
	*/
	...
	for (vector<vector<Point>>::iterator iterator = res.begin(), int i = 0; iterator != res.end() && i < lineslength.size(); ++iterator, ++i) {
...
vector<vector<Point>> connectCollineationLine(vector<vector<Point>>& input, vector <double >lengths_out) {
	...
	for (int i = 0; i < linesPolarData.size(); i++) {
		...
		for (int j = i + 1; j < linesPolarData.size(); j++) {
			...
			if (abs(itemPolarDataFirst.first - itemPolarDataSecond.first) < itemPolarDataFirst.first * threshold_r
