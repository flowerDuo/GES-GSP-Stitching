...
public:
	...
	const vector<vector<double> >& getSamplesWeight() const;
	...
	const vector<vector<vector<InterpolateVertex> > >& getContentMeshInterpolation() const;
...
private:
	...
	//#获取该图像对的ransac误差。
	const double generateRansacAvgDiff(const pair<int, int>& _index_pair) const;
	//#将得到的图像对的ransac误差均值进行数据处理，得到权值数据。
	const void generateRansacDiffWeight(vector<pair<int, int> > pair) const;
	...
	mutable vector<vector<double> > images_polygon_samples_weight;
