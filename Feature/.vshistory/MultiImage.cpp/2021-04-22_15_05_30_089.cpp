...
/// </summary>
/// <returns></returns>
const vector<vector<vector<InterpolateVertex>>>& MultiImages::getContentMeshInterpolation() const
{
...
const vector<vector<double> >& MultiImages::getSamplesWeight() const {
	double throsholdMin = 0.2;

	images_polygon_samples_weight.resize(images_data.size());
	...
	//
	for (int i = 0; i < images_polygon_samples_weight.size(); ++i) {//图像个数,遍历每个图像
		//一个图像的网格数量
		const int polygons_count = (int)images_data[i].mesh_2d->getPolygonsIndices().size();
		...
		//每个网格的权重
		images_polygon_samples_weight[i].reserve(polygons_count);
		...
		for (int j = 0; j < polygons_has_matching_pts.size(); ++j) {
			if (polygons_has_matching_pts[j]) {
				polygons_has_matching_pts[j] = false;
				images_polygon_samples_weight[i].emplace_back(0);
				que.push(dijkstraNode(j, j, 0));
			}
			else {
				images_polygon_samples_weight[i].emplace_back(FLT_MAX);
			}
		}
		...
		while (que.empty() == false) {//队列不为空,就一直循环.
			...
			if (polygons_has_matching_pts[index] == false) {
				...
				for (int j = 0; j < polygons_neighbors[index].indices.size(); ++j) {//循环得到此网格左上角顶点的邻居点index
					...
					if (polygons_has_matching_pts[n] == false) { //判断此网格的 一个邻居网格mask是否为false(即这个网格没有被处理过)
						const double dis = norm(polygons_center[n] - polygons_center[now.from]);
						if (images_polygon_samples_weight[i][n] > dis) {
							images_polygon_samples_weight[i][n] = dis;
							que.push(dijkstraNode(now.from, n, dis));
						}
					}
				}
		...
		double maxDist = *max_element(images_polygon_samples_weight[i].begin(), images_polygon_samples_weight[i].end());

		//将得到的权重数据进行归一化
		for (int j = 0; j < images_polygon_samples_weight[i].size(); ++j) {
			//数据处理
			images_polygon_samples_weight[i][j] = exp(-pow(1 - images_polygon_samples_weight[i][j] * maxDist, 2) / (1 / log(1 / throsholdMin)));
		}

	}

	return images_polygon_samples_weight;
}
