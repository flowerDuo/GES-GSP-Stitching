		...
		for (int i = 0; i < images_polygon_distance_to_nonOverlap.size(); ++i) {//图像个数,遍历每个图像
			...
			for (int j = 0; j < oneImageSamples.size(); j++) {
				...
				for (int k = 0; k < oneImageSamples[j].size(); k++) {
					...
					if (distanceToNonOverlop == 0) {//.0:就在非重叠区域.
			...
			double maxDist = *max_element(images_polygon_distance_to_nonOverlap[i].begin(), images_polygon_distance_to_nonOverlap[i].end());

			//将得到的权重数据进行归一化
			for (int j = 0; j < images_polygon_distance_to_nonOverlap[i].size(); ++j) {
				//数据处理
				images_polygon_distance_to_nonOverlap[i][j] = exp(-pow(images_polygon_distance_to_nonOverlap[i][j] / maxDist, 2) / (1 / log(1 / throsholdMin)));
			}
