...
Mat MultiImages::textureMapping(const vector<vector<Point2> >& _vertices,
	...
	for (int i = 0; i < images_data.size(); ++i) {//遍历图像
		...
		imwrite(parameter.debug_dir + parameter.file_name + images_data[i].file_name + "_item_wraping.png", image);
	}
		...
		for (int i = 0; i < images_polygon_distance_to_nonOverlap.size(); ++i) {//图像个数,遍历每个图像
			...
			for (int j = 0; j < oneImageSamples.size(); j++) {
				...
				for (int k = 0; k < oneImageSamples[j].size(); k++) {
					...
					//double sampleWeight = exp(-(pow(ratio, 2) / 2 * 0.2));
					//double sampleWeight = -ratio + 1;
				...
				/*for (int k = 0; k < oneImageSamples[j].size(); k++) {
					if (maxWeight >= 0.75) {
						...
						samplesWeight[i][j][k] = samplesWeight[i][j][k] / 0.75;
					}
					else {
						samplesWeight[i][j][k] = samplesWeight[i][j][k] / 0.5;
					}
				}*/
			}
		}
