...
double MultiImages::getWarpingResidual(vector<vector<Point2>> _vertices) const
{
	for (int i = 0; i < images_data.size(); ++i) {
		...
		for (int j_row = 0; j_row < rows.size(); j_row++) {
			...
			getLineAvgResidual(row_item, line_para);
		}
...
/// <returns></returns>
pair<double, double> getLineAvgResidual(vector<Point2f> _vertices, Vec4f _line_param)
{
	...
	double avg = sum / _vertices.size();
	double s = 0;
	for (int i = 0; i < residuals.size(); i++) {
		s += pow(residuals[i] - avg, 2) / residuals.size();
	}
	s = sqrt(s);
	return pair<double>(avg, s);
}
