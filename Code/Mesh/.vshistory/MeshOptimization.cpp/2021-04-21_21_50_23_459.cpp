...
void MeshOptimization::prepareContentPreservingTerm(vector<Triplet<double> >& _triplets,
	...
	if (content_preserving_equation.second) {
		//直线采样点坐标转化为网格插值格式.
		const vector<vector<vector<InterpolateVertex> > >& content_interpolation = multi_images->getContentMeshInterpolation();
		//todo
		const vector<vector<double> >& images_grid_space_matching_pts_weight = multi_images->getSamplesWeight();
		//存放 UV 数据
					...
					for (int dim = 0; dim < DIMENSION_2D; ++dim) { //采样点的 x,y
						for (int p = 0; p < multi_images->images_data[i].mesh_2d->getPolygonVerticesCount(); ++p) {//遍历每个网格的4个点
							...
							_triplets.emplace_back(content_preserving_equation.first + eq_count + dim,
								...
								content_interpolation[i][j][k].weights[p] * content_changed_weight);
							...
							if (dim == 0) { // x 的方程式
								...
								_triplets.emplace_back(content_preserving_equation.first + eq_count + dim,
									...
									-(1 - content_term_uv[i][j][k - 2].first) * content_interpolation[i][j][0].weights[p] * content_changed_weight);
								...
								_triplets.emplace_back(content_preserving_equation.first + eq_count + dim,
									...
									content_term_uv[i][j][k - 2].second * content_interpolation[i][j][0].weights[p] * content_changed_weight);
								...
								_triplets.emplace_back(content_preserving_equation.first + eq_count + dim,
									...
									-content_term_uv[i][j][k - 2].first * content_interpolation[i][j][1].weights[p] * content_changed_weight);
								...
								_triplets.emplace_back(content_preserving_equation.first + eq_count + dim,
									...
									-content_term_uv[i][j][k - 2].second * content_interpolation[i][j][1].weights[p] * content_changed_weight);
							}
							else {
								_triplets.emplace_back(content_preserving_equation.first + eq_count + dim,
